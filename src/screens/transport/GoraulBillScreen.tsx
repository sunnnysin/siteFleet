...
import {
  fetchGoraulVehicleRates,
  buildGoraulVehicleRateMap,
  setGoraulVehicleRate,
} from '@/services/goraulVehicleRateService';
import {
  shareGoraulBillReport,
  GORAUL_VEHICLE_TYPE_BILL_LABELS,
} from '@/services/goraulBillReportService';
...
type GoraulBillScreenProps = NativeStackScreenProps<
  TransportStackParamList,
  'GoraulBill'
>;

export function GoraulBillScreen({ navigation }: GoraulBillScreenProps) {
  const [selectedMonth, setSelectedMonth] = useState(currentMonthKey());
  const [hasSummaryForMonth, setHasSummaryForMonth] = useState(false);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [rates, summary] = await Promise.all([
        fetchGoraulVehicleRates(),
        fetchGoraulSummary(selectedMonth),
      ]);
      const rateMap = buildGoraulVehicleRateMap(rates);
      setRateInputs(
        Object.fromEntries(
          VEHICLE_TYPES.map(type => [type, String(rateMap[type])]),
        ) as Record<VehicleType, string>,
      );
      setHasSummaryForMonth(summary !== null);
      const totals = computeGoraulSummaryTotals(summary?.entries ?? []);
      setVehicleTotals({
        ACE: totals.ace,
        'Bolero/PickUp': totals.bolero,
      });
    } catch (error) {
      setErrorMessage(
  ...
  async function handleSaveRates(): Promise<void> {
    ...
    try {
      await Promise.all(parsedRates.map(rate => setGoraulVehicleRate(rate)));
    } catch (error) {
      setSaveErrorMessage(
  ...
  async function handleExport(): Promise<void> {
    ...
    try {
      ...
      await shareGoraulBillReport(monthLabel, lineItems);
    } catch (error) {
    ...
    return (
      <SafeAreaView style={styles.container} edges={[]}>
        ...
        <EmptyState
          ...
          message={`Kindly add the Goraul ${monthLabel} summary first to see the ${monthLabel} bill.`}
          retryLabel="Go to Goraul Summary"
          onRetry={() => navigation.navigate('GoraulSummary')}
        />
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {VEHICLE_TYPES.map(type => (
          <FormTextInput
            key={type}
            label={GORAUL_VEHICLE_TYPE_BILL_LABELS[type]}
          ...
          {VEHICLE_TYPES.map(type => (
            <View key={type} style={styles.tableRow}>
              <Text style={styles.tableCell}>
                {GORAUL_VEHICLE_TYPE_BILL_LABELS[type]}
              </Text>
